...
<section class="categories">
    <div class="container">
        ...
        <div class="categories__items">
            ...
            // Получаем все категории, где top(не подкатегории) пусто
            $sql = "SELECT c.id, c.title, c.icon, COUNT(p.id) AS product_count
                FROM categories c
                LEFT JOIN products p ON p.categories_id = c.id
                WHERE c.top IS NULL OR c.top = ''
                GROUP BY c.id";
            ...
            <?php foreach ($categories as $category): ?>
                <div class="categories__item">
                    <div class="categories__item_icon">
                        <img src="<?= htmlspecialchars($category['icon']) ?>" alt="">
                    </div>
                    <h4 class="categories__item_title">
                        <a href="category.php?id_category=<?= (int)$category['id'] ?>">
                            <?= htmlspecialchars($category['title']) ?>
                        </a>
                    </h4>
                    <div class="categories__item_count"><?= $category['product_count'] ?> шт.</div>
                </div>
            <?php endforeach; ?>
        ...
        <div class="swiper sale-carousel">
            <div class="swiper-wrapper">
                ...
                <?php foreach ($sales as $product): ?>
                    <form action="" method="POST" class="swiper-slide mini-product">
